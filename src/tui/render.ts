...
const liveOutputSignalPattern = /\b(?:access denied|denied|error|exception|fail(?:ed|ure)?|fatal|panic|rejected|timeout|timed out|unable|warning)\b|\bE[A-Z0-9_]{2,}\b/;
...
function hasLiveOutputSignal(line: string): boolean {
	return liveOutputSignalPattern.test(oneLine(line));
}
