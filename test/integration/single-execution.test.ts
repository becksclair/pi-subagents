...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("rejects duplicate concurrent top-level subagent execution calls", async () => {
	...
	it("allows management actions while an execution call is in progress", async () => {
		...
		assert.doesNotMatch(status.content[0]?.text ?? "", /already in progress/);
		assert.equal(mockPi.callCount(), 1);
	});
	...
	it("retries with fallback models after an empty child response", async () => {
		mockPi.onCall({ output: "" });
		mockPi.onCall({ output: "Recovered after cold start" });
		...
		assert.equal(result.exitCode, 0);
		assert.equal(result.finalOutput, "Recovered after cold start");
		assert.deepEqual(result.attemptedModels, ["openai/gpt-5-mini", "anthropic/claude-sonnet-4"]);
		assert.match(result.modelAttempts?.[0]?.error ?? "", /produced no output/);
